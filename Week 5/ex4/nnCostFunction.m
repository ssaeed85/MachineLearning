...
% Create super matrix Y where Y(i,:) is the actual output for the i-th 
% training set (i-th row of Y)
Y = zeros(size(y,1),num_labels);
for i = 1:m
Y(i,y(i)) = 1;
end

% Matricised(?) form of cost function
J = 1/m * ( -Y .* log(h) - (1-Y) .* log(1-h));
J = sum(J(:)); % Adds all elements of matrix

%-------------------------------------
% Calculating regularization element

Reg = 0;
Theta1Squared = Theta1(:,2:end) .^ 2; % ignore first (bias) column
Theta2Squared = Theta2(:,2:end) .^ 2;
Reg = lambda/(2*m) * (sum(Theta1Squared(:))+sum(Theta2Squared(:)));
